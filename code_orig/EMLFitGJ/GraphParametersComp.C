...
void GraphParametersComp(TString DirName,TString Var,int systemNum, int binType){
  ...
  for(Int_t ib=0;ib<DataBins->GetN();ib++){
    ...
    cout<<"Lou4"<<redName<<" "<<axisBin<<" "<<iP<<" "<<va<<endl;
    ...
    cout<<"Lou4.1 "<<DirName+TString("Results")+DataBins->GetBinName(ib)+".root"<<endl;
   if(!fileR->IsOpen()) continue;
    ...
    for(Int_t ipar=0;ipar<Pars.getSize();ipar++){
      ...
      cout<<"Lou5"<<redName<<" "<<DataBins->GetAxis(va).GetBinCenter(iP+1)<<endl;
  ...
  for (int i=0; i<10; i++) {
	...
	if (binType==0) {
	  g->GetXaxis()->SetRangeUser(0.1,1.4);
	}
	const char *title = "";
	...
	legend = new TLegend(0.5,0.65,0.98,0.85);	
	//                   x1  y1  x2   y2
	if (i==6) {
		TString name = g->GetName();
		TString label = name(0,name.Index("_"));
		legend->AddEntry(name,label,"p");
		name = g2->GetName();
		label = name(0,name.Index("_"));
		legend->AddEntry(name,label,"p");
		legend->Draw();
	}
  ...
  c1->SaveAs(DirName+graphTitle+" "+Var+".png");
  ...
  if (binType==2) { // energy bins
	 ...
	 c2->SaveAs(DirName+graphTitle+"5vars_"+Var+".png");  
