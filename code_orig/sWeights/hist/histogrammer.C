...
Bool_t histogrammer::Process(Long64_t entry)
{
 ...
 //below you can give vars corresponding to fBins axis
   ...
   if(fBins) fCurrBin=fBins->FindBin(*cosThetaPhiCm, *top);
   FillHistograms("All");
...
void histogrammer::HistogramList(TString sLabel){
  TDirectory::AddDirectory(kFALSE); //do not add to current directory
	...
	fOutput->Add(MapHist(new TH2F("mass_KpKm_pKm","mass_KpKm_pKm", 200, 0.7, 1.8, 200, 1.2, 2.1)));
fOutput->Add(MapHist(new TH1F("t","t", 100, 0.0, 2.5)));
	
	// pPipPim plots
	fOutput->Add(MapHist(new TH1F("masspPip","masspPip", 1000, 1.0, 2.0)));
	fOutput->Add(MapHist(new TH1F("masspPim","masspPim", 1000, 1.0, 2.0)));
	fOutput->Add(MapHist(new TH2F("mass_pPip_KpKm","mass_pPip_KpKm", 200, 1.0, 2.0, 200, 0.7, 1.8)));
	fOutput->Add(MapHist(new TH2F("mass_pPim_KpKm","mass_pPim_KpKm", 200, 1.0, 2.0, 200, 0.7, 1.8)));
	...
	// K+
	fOutput->Add(MapHist(new TH1F("phiKpCmPARA","phiKpCmPARA", 100, -3.2, 3.2)));
	fOutput->Add(MapHist(new TH1F("phiKpCmPERP","phiKpCmPERP", 100, -3.2, 3.2)));
	// K-
	fOutput->Add(MapHist(new TH1F("phiKmCmPARA","phiKmCmPARA", 100, -3.2, 3.2)));
	fOutput->Add(MapHist(new TH1F("phiKmCmPERP","phiKmCmPERP", 100, -3.2, 3.2)));
	// Lambda
	fOutput->Add(MapHist(new TH1F("phiLamCmPARA","phiLamCmPARA", 100, -3.2, 3.2)));
	fOutput->Add(MapHist(new TH1F("phiLamCmPERP","phiLamCmPERP", 100, -3.2, 3.2)));
...
void histogrammer::FillHistograms(TString sCut){ 
  ...
  ((TH2F*)FindHist("mass_KpKm_pKm"))->Fill(*MKpKm, *MpKm, fWeight);
  FindHist("t")->Fill(*t,fWeight);
  FindHist("phiPhimPhiProton")->Fill(*phiPhimPhiProton,fWeight);
  
  FindHist("masspPip")->Fill(*MpPip,fWeight);
  FindHist("masspPim")->Fill(*MpPim,fWeight);
  ((TH2F*)FindHist("mass_pPip_KpKm"))->Fill(*MpPip, *MKpKm, fWeight);
  ((TH2F*)FindHist("mass_pPim_KpKm"))->Fill(*MpPim, *MKpKm, fWeight);
  ...
  if (*polState==1) {
	  ...
	  FindHist("phiKpCmPARA")->Fill(*phiKpCm,fWeight);
	  FindHist("phiKmCmPARA")->Fill(*phiKmCm,fWeight);
	  FindHist("phiLamCmPARA")->Fill(*phiLamCm,fWeight);
  ...
  if (*polState==-1) {
	  ...
	  FindHist("phiKpCmPERP")->Fill(*phiKpCm,fWeight);
	  FindHist("phiKmCmPERP")->Fill(*phiKmCm,fWeight);
	  FindHist("phiLamCmPERP")->Fill(*phiLamCm,fWeight);
	...
	if (*MpKm >1.5 && *MpKm < 1.54 && *MKpKm > 1.04) {
		...
		if (*polState==-1) {
			FindHist("LphiPhiCmPERP")->Fill(*phiPhiCm,fWeight);
			FindHist("LphiKpCmPERP")->Fill(*phiKpCm,fWeight);
			FindHist("LphiKmCmPERP")->Fill(*phiKmCm,fWeight);
			FindHist("LphiLamCmPERP")->Fill(*phiLamCm,fWeight);
		}
	...
	if (*MpKm >1.54 && *MKpKm < 1.04 && *MKpKm > 1.0) {
		...
		if (*polState==-1) {
			FindHist("PphiPhiCmPERP")->Fill(*phiPhiCm,fWeight);
			FindHist("PphiKpCmPERP")->Fill(*phiKpCm,fWeight);
			FindHist("PphiKmCmPERP")->Fill(*phiKmCm,fWeight);
			FindHist("PphiLamCmPERP")->Fill(*phiLamCm,fWeight);
		}
	}
